...
```bash
AWS$ sam local invoke MarketDataTokenFunction --event events/event.json
```
...
```bash
AWS$ sam logs -n MarketDataTokenFunction --stack-name AWS --tail
```
